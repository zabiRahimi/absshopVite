...
const City = ({ sendCity, stateName, id, name, required, error, value }) => {

    const wrapper_Ci = useRef(null);
    const iStar_Ci = useRef(null);
    const iTrue_Ci = useRef(null);
    const iFalse_Ci = useRef(null);
    const textarea = useRef(null);
    const label_Ci = useRef(null);
    const errorDiv_Ci = useRef(null);
    const defaultOption = useRef(null);

    useEffect(() => {


        required && iStar_Ci.current.classList.remove('--displayNone');
    ...
    // هنگامی که کاربر دوباره اقدام به انتخاب استان می‌کند
    useEffect(() => {
    ...
    const handleShowLabel = (type) => {
        ...
        if (type == 'show') {
            ...
            label_Ci.current.classList.remove('--hidden');

        } else {

            label_Ci.current.classList.add('--hidden');
    ...
    // هنگامی که کاربر گزینه انتخاب شهر را کلیک می‌کند
    const handleFocus = () => {

        handleShowLabel('hide');

        wrapper_Ci.current.classList.remove('greenBorder_Fo');

        wrapper_Ci.current.classList.remove('redBorder_Fo');

        iTrue_Ci.current.classList.add('--displayNone');

        iFalse_Ci.current.classList.add('--displayNone');

        required && iStar_Ci.current.classList.remove('--displayNone');

        errorDiv_Ci.current.classList.add('--displayNone');
    ...
    const borderStyle = (colorBorder) => {
        ...
        wrapper_Ci.current.classList.add(colorBorder);
    ...
    const hiddenStarIcon = () => {
        ...
        iStar_Ci.current.classList.add('--displayNone');
        ...
        switch (stateName) {
            ...
            default:
                return [
                    ...
                    <option className='alertSelectState_Fo' key='2' disabled > ابتدا استان را انتخاب کنید !! </option>
                ]
                    ;
    ...
    return (
        <div className="containerWrapper_Fo">
            ...
            <div className='wrapper_Fo' ref={wrapper_Ci}>
                <i className='icofont-ui-rating --displayNone iStar_Fo' ref={iStar_Ci} />

                <i className="icofont-tick-mark --displayNone iTrue_Fo" ref={iTrue_Ci} />

                <i className='icofont-exclamation --displayNone iFalse_Fo' ref={iFalse_Ci} />

                <label htmlFor='state' className='label_Fo --hidden' ref={label_Ci}>
                ...
                <select className="select_Fo" id={id} name={name} onChange={(e) => { handleChange(e) }}>
                    <option defaultValue='' ref={defaultOption} onClick={handleFocus} selected>انتخاب شهر</option>
                    {option()}
                </select>

            </div>

            <div className='errorDiv_Fo --hidden' ref={errorDiv_Ci}> {error} </div>
        </div>
